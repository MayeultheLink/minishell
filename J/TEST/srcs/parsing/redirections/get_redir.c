...
/*   Updated: 2022/01/04 18:30:49 by mde-la-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//printf("io2 = %d\n", io);
//printf("str = %s\n", lst->token->str);
	if (lst->token->str && lst->token->str[0] && lst->token->str[1] && (lst->token->str[1] == '<' || lst->token->str[1] == '>'))
		(*type_redir)++;
...
t_lst	*get_redir(t_lst *lst)
{
	...
	while (lst)
	{
		...
		if (in >= 0)
			lst_cmd->token->redir_in = fill_redir(ft_lststart(lst), &lst->token->type_redir_in, in);
//	printf("redir in = %s\n", lst->token->redir_in);
//	printf("out = %d\n", out);
		if (out >= 0)
			lst_cmd->token->redir_out = fill_redir(ft_lststart(lst), &lst->token->type_redir_out, out);
//	printf("redir out = %s\n", lst->token->redir_out);
