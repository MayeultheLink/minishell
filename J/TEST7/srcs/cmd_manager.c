...
/*   Updated: 2022/01/06 16:10:21 by jpauline         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//printf("BF :: in F%d tab_pid=%d, cmd=%s, fd[1]=%d,fd[0]=%d\n", i, tab_pid[i - 1], node->token->cmd[0], tab_fd[0], tab_fd[1]);
		tab_pid[i - 1] = fork();
		if (tab_pid[i - 1] == -1)
			return (1); /*error*/
		if (tab_pid[i - 1] == 0)
		{
//printf("CMD :: in F%d tab_pid=%d, cmd=%s\n", i, tab_pid[i - 1], node->token->cmd[0]);
